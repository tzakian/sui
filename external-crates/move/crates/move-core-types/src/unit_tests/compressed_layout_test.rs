...
fn assert_runtime_deser_parity(layout: &R::MoveTypeLayout, value: &R::MoveValue) {
    ...
    let compressed_result =
        R::MoveValue::simple_deserialize_compressed(&blob, &compressed).unwrap();
    assert_eq!(
...
fn assert_annotated_deser_parity(layout: &A::MoveTypeLayout, blob: &[u8]) {
    ...
    let compressed = AC::MoveTypeLayout::from(layout);
    let compressed_result = A::MoveValue::simple_deserialize_compressed(blob, &compressed).unwrap();
    assert_eq!(
        tree_result, compressed_result,
