...
use crate::annotated_value::{MoveFieldLayout, MoveTypeLayout as TreeMoveTypeLayout};
...
/// A deduplicated, flat representation of an annotated [`TreeMoveTypeLayout`] tree.
...
// --- Owned layout types (resolved from the shared pool) ---
...
pub enum VariantLayout {
    ...
    Unknown { name: Identifier, tag: u16 },
}
...
impl MoveLayoutView {
    ...
    pub fn inflate(&self) -> AResult<TreeMoveTypeLayout> {
        ...
        Ok(match self {
            ...
            MoveLayoutView::Vector(vv) => TreeMoveTypeLayout::Vector(Box::new(vv.inflate()?)),
            MoveLayoutView::Struct(sv) => {
                let fields = sv
                    .fields()
                    .map(|(name, layout)| Ok(MoveFieldLayout::new(name.clone(), layout.inflate()?)))
                    .collect::<AResult<_>>()?;
                TreeMoveTypeLayout::Struct(Box::new(TreeStructLayout {
            ...
            MoveLayoutView::Enum(ev) => {
                let variants = ev
                    ...
                    .map(|vl| match vl.fields() {
                        Some(fields) => {
                            let field_layouts = fields
                                .fields()
                                .map(|(name, layout)| {
                                    Ok(MoveFieldLayout::new(name.clone(), layout.inflate()?))
                                })
                                .collect::<AResult<_>>()?;
                    ...
                    .collect::<AResult<_>>()?;
                TreeMoveTypeLayout::Enum(Box::new(TreeEnumLayout {
...
impl MoveFieldsLayout {
    ...
    pub fn field(&self, i: usize) -> Option<(&Identifier, MoveTypeLayout)> {
        self.fields.get(i).map(|(name, layout_ref)| {
            (
                name,
                MoveTypeLayout {
    ...
    pub fn field_by_name(&self, name: &str) -> Option<MoveTypeLayout> {
        self.fields
            ...
            .map(|(_, layout_ref)| MoveTypeLayout {
                pool: self.pool.clone(),
                root: *layout_ref,
            })
    }
    ...
    pub fn fields(&self) -> impl ExactSizeIterator<Item = (&Identifier, MoveTypeLayout)> {
        let pool = &self.pool;
        self.fields.iter().map(move |(name, layout_ref)| {
            (
                name,
                MoveTypeLayout {
                    pool: pool.clone(),
...
impl MoveStructLayout {
    ...
    pub fn field_view(&self) -> &MoveFieldsLayout {
        &self.fields
    }
...
impl MoveEnumLayout {
    ...
    pub fn variant_by_tag(&self, tag: u16) -> Option<&VariantLayout> {
        ...
        ResolvedRef::Index(idx) => match &pool[idx] {
            ...
            MoveTypeNode::Enum(e) => MoveLayoutView::Enum(Box::new(MoveEnumLayout {
                ...
                variants: e
                    .variants
                    .iter()
                    .map(|(name, tag, fields)| match fields {
