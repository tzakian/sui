...
impl MoveTypeLayoutBuilder {
    ...
    pub fn from_layout(&mut self, layout: &MoveTypeLayout) -> AResult<LayoutHandle> {
        self.from_view(layout.as_view())
    }

    fn from_view(&mut self, view: MoveLayoutView) -> AResult<LayoutHandle> {
        Ok(match view {
            ...
            MoveLayoutView::Enum(e) => {
                let variants: Vec<(Identifier, VariantTag, Option<Vec<(Identifier, LayoutHandle)>>)> = e
                    .variants()
                    .iter()
                    .map(|v| match v {
                        VariantLayout::Known { name, tag, fields } => {
