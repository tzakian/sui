...
fn field_handles(
    ...
    let field_handles = module
        ...
        .map(|f_handle| {
            ...
            let owner = structs.at(def_idx.0 as usize)?.def_vtable_key.clone();
...
fn variant_handles(
    ...
    module
        ...
        .map(|variant_handle| {
            ...
            let enum_ = enums.at(enum_def.0 as usize)?;
            let variant_ = enum_.variants.at(*variant as usize)?;
...
fn struct_instantiations(
    ...
    let struct_insts = module
        ...
        .map(|struct_inst| {
            ...
            let struct_def = &structs.at(def)?;
            let field_count = struct_def.fields.len() as u16;
            let instantiation_idx = struct_inst.type_parameters;
            let type_params = signatures.at(instantiation_idx.0 as usize)?.ptr_clone();
    ...
    let fun_insts = module
        ...
        .map(|fun_inst| {
            ...
            let instantiation = signatures
                .at(fun_inst.type_parameters.0 as usize)?
                .ptr_clone();
...
fn field_instantiations(
    ...
    let field_instantiations = module
        ...
        .map(|f_inst| {
            ...
            let owner = field_handles.at(fh_idx.0 as usize)?.owner.clone();
            let offset = field_handles.at(fh_idx.0 as usize)?.offset;
...
fn variant_instantiations(
    ...
    let variant_insts = module
        ...
        .map(|v_inst| {
            ...
            let enum_inst = VMPointer::from_ref(enum_instantiations.at(enum_def.0 as usize)?);
            let variant = VMPointer::from_ref(enum_inst.enum_def.variants.at(*variant as usize)?);
...
fn bytecode(
    ...
    let bytecode = match bytecode {
        ...
        input::Bytecode::LdConst(ndx) => {
            let const_ptr = &context.definitions.constants.at(ndx.0 as usize)?;
        ...
        input::Bytecode::Pack(ndx) => {
            let struct_ptr = &context.definitions.structs.at(ndx.0 as usize)?;
        ...
        input::Bytecode::MutBorrowField(ndx) => {
            let field_ptr = &context.definitions.field_handles.at(ndx.0 as usize)?;
        ...
        input::Bytecode::PackVariant(ndx) => {
            let variant_ptr = &context.definitions.variants.at(ndx.0 as usize)?;
        ...
        input::Bytecode::VariantSwitch(ndx) => {
            let jump_table = &jump_tables.at(ndx.0 as usize)?;
