...
impl VMDispatchTables {
    ...
    ) -> PartialVMResult<RC::LayoutHandle> {
        type_size.check()?;
        let ty = self.resolve_type(datatype_name)?.to_ref();
        let handle = match ty.datatype_info.inner_ref() {
            Datatype::Enum(einfo) => {
                let mut variant_handles: Vec<Option<Vec<RC::LayoutHandle>>> = vec![];
                for variant in einfo.variants.iter() {
                    ...
                    let fields = field_tys
                        .iter()
                        .map(|ty| self.type_to_type_layout_impl(ty, type_size, builder))
                        .collect::<PartialVMResult<Vec<_>>>()?;
                    variant_handles.push(Some(fields));
                }
                let variants: Vec<Option<&[RC::LayoutHandle]>> =
                    variant_handles.iter().map(|v| v.as_deref()).collect();
                builder.enum_layout(&variants)
            }
            Datatype::Struct(sinfo) => {
                ...
                let fields = field_tys
                    .iter()
                    .map(|ty| self.type_to_type_layout_impl(ty, type_size, builder))
                    .collect::<PartialVMResult<Vec<RC::LayoutHandle>>>()?;
                builder.struct_layout(&fields)
            }
        };
        type_size.check()?;
        Ok(handle)
    ...
    fn type_to_type_layout_impl(
        ...
        type_size.enter_type(|type_size| {
            let handle = match ty {
                ...
                Type::Vector(ty) => {
                    let inner = self.type_to_type_layout_impl(ty, type_size, builder)?;
                    builder.vector(inner)
                }
                Type::Datatype(gidx) => {
                    self.datatype_to_type_layout(gidx, &[], type_size, builder)?
                }
                Type::DatatypeInstantiation(inst) => {
                    let (gidx, ty_args) = &**inst;
                    self.datatype_to_type_layout(gidx, ty_args, type_size, builder)?
                }
                Type::Reference(_) | Type::MutableReference(_) | Type::TyParam(_) => {
                    return Err(partial_vm_error!(
                        UNKNOWN_INVARIANT_VIOLATION_ERROR,
    ...
    ) -> PartialVMResult<AC::LayoutHandle> {
        type_size.check()?;
        let ty = self.resolve_type(datatype_name)?.to_ref();
        ...
        let handle = match ty.datatype_info.inner_ref() {
            Datatype::Enum(enum_type) => {
                let mut owned_variants: Vec<(
                    Identifier,
                    u16,
                    Vec<(Identifier, AC::LayoutHandle)>,
                )> = vec![];
                for variant in enum_type.variants.iter() {
                    ...
                    let fields = variant
                        ...
                        .map(|(n, ty)| {
                            ...
                            let h =
                                self.type_to_fully_annotated_layout_impl(&ty, type_size, builder)?;
                            Ok((n, h))
                        })
                        .collect::<PartialVMResult<Vec<_>>>()?;
                    owned_variants.push((variant_name, variant.variant_tag, fields));
                }
                ...
                let variant_refs: Vec<(
                    &Identifier,
                    u16,
                    Option<&[(&Identifier, AC::LayoutHandle)]>,
                )> = owned_variants
                    .iter()
                    .zip(field_ref_vecs.iter())
                    .map(|((vn, tag, _), fields)| (vn, *tag, Some(fields.as_slice())))
                    .collect();
                builder.enum_layout(&struct_tag, &variant_refs)
            }
            Datatype::Struct(struct_type) => {
                ...
                let field_refs: Vec<(&Identifier, AC::LayoutHandle)> =
                    fields.iter().map(|(n, h)| (n, *h)).collect();
                builder.struct_layout(&struct_tag, &field_refs)
            }
        };
        type_size.check()?;
        Ok(handle)
    ...
    fn type_to_fully_annotated_layout_impl(
        ...
        type_size.enter_type(|type_size| {
            let handle = match ty {
                ...
                Type::Signer => builder.signer(),
                Type::Vector(ty) => {
                    let inner = self.type_to_fully_annotated_layout_impl(ty, type_size, builder)?;
                    builder.vector(inner)
                }
                Type::Datatype(gidx) => {
                    self.datatype_to_fully_annotated_layout_impl(gidx, &[], type_size, builder)?
                }
                Type::DatatypeInstantiation(inst) => {
                    let (gidx, ty_args) = &**inst;
                    self.datatype_to_fully_annotated_layout_impl(gidx, ty_args, type_size, builder)?
                }
                Type::Reference(_) | Type::MutableReference(_) | Type::TyParam(_) => {
                    return Err(partial_vm_error!(
                        UNKNOWN_INVARIANT_VIOLATION_ERROR,
    ...
    pub(crate) fn type_to_type_layout(&self, ty: &Type) -> PartialVMResult<RC::MoveTypeLayout> {
        let mut builder = RC::MoveTypeLayoutBuilder::new();
        let root = self.type_to_type_layout_impl(
            ty,
            &mut TypeSize::from_vm_config_for_value_depth(&self.vm_config),
            &mut builder,
        )?;
    ...
    pub(crate) fn arena_type_to_fully_annotated_layout(
        &self,
        ty: &ArenaType,
    ) -> PartialVMResult<AC::MoveTypeLayout> {
    ...
    pub(crate) fn type_to_fully_annotated_layout(
        ...
        let root = self.type_to_fully_annotated_layout_impl(
            ty,
            &mut TypeSize::from_vm_config_for_value_depth(&self.vm_config),
            &mut builder,
        )?;
    ...
    pub(crate) fn get_type_layout(&self, type_tag: &TypeTag) -> VMResult<RC::MoveTypeLayout> {
