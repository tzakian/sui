...
impl SafeIndex<MemBox<Value>> for FixedSizeVec {
    fn at(&self, index: usize) -> PartialVMResult<&MemBox<Value>> {
        self.0.get(index).ok_or_else(|| {
            partial_vm_error!(
                INDEX_OUT_OF_BOUNDS,
                "Index {} out of bounds for fixed-size vector of length {}",
                index,
                self.len()
            )
        })
        ...
        match (self, other) {
            ...
            (Reference::Indexed(ref_1), Reference::Indexed(ref_2)) => {
                ...
                match_prim_vec_pair!(
                    ...
                    items1,
                    items2,
                    items1.at(*ndx_1)? == items2.at(*ndx_2)?,
                    partial_vm_error!(
                        INTERNAL_TYPE_ERROR,
                        "cannot compare values: {:?}, {:?}",
                        self,
            ...
            | (Reference::Indexed(entry), Reference::Value(mem_box)) => {
                ...
                match (vec, box_value) {
                    (PrimVec::VecU8(lhs), Value::U8(rhs)) => Ok(lhs.at(*ndx)? == rhs),
                    (PrimVec::VecU16(lhs), Value::U16(rhs)) => Ok(lhs.at(*ndx)? == rhs),
                    (PrimVec::VecU32(lhs), Value::U32(rhs)) => Ok(lhs.at(*ndx)? == rhs),
                    (PrimVec::VecU64(lhs), Value::U64(rhs)) => Ok(lhs.at(*ndx)? == rhs),
                    (PrimVec::VecU128(lhs), Value::U128(rhs)) => Ok(lhs.at(*ndx)? == &**rhs),
                    (PrimVec::VecU256(lhs), Value::U256(rhs)) => Ok(lhs.at(*ndx)? == &**rhs),
                    (PrimVec::VecBool(lhs), Value::Bool(rhs)) => Ok(lhs.at(*ndx)? == rhs),
                    (PrimVec::VecAddress(lhs), Value::Address(rhs)) => Ok(lhs.at(*ndx)? == &**rhs),
                    _ => Err(partial_vm_error!(
                        INTERNAL_TYPE_ERROR,
                        "cannot compare values: {:?}, {:?}",
                        self,
    ...
    pub fn borrow_field(&self, index: usize) -> PartialVMResult<Value> {
        ...
        match &*container {
            ...
            Value::Struct(fixed_vec) => {
                ...
                let field: &MemBox<Value> = fixed_vec.at(index)?;
...
// Implement the `Index` trait to allow immutable indexing.
impl SafeIndex<MemBox<Value>> for Struct {
    fn at(&self, index: usize) -> PartialVMResult<&MemBox<Value>> {
        self.0.at(index)
    }
}
        ...
        match self {
            ...
            Reference::Indexed(index_ref) => {
                ...
                match_prim_vec!(
                    vec,
                    vec,
                    write!(f, "{}", vec.at(*ndx).map_err(|_| fmt::Error)?)
                )
            }
        }
    }
...
impl PrimVec {
    ...
    fn visit_indexed(
        ...
        match self {
            PrimVec::VecU8(xs) => visitor.visit_u8(depth, *xs.at(ndx)?),
            PrimVec::VecU16(xs) => visitor.visit_u16(depth, *xs.at(ndx)?),
            PrimVec::VecU32(xs) => visitor.visit_u32(depth, *xs.at(ndx)?),
            PrimVec::VecU64(xs) => visitor.visit_u64(depth, *xs.at(ndx)?),
            PrimVec::VecU128(xs) => visitor.visit_u128(depth, *xs.at(ndx)?),
            PrimVec::VecU256(xs) => visitor.visit_u256(depth, *xs.at(ndx)?),
            PrimVec::VecBool(xs) => visitor.visit_bool(depth, *xs.at(ndx)?),
            PrimVec::VecAddress(xs) => visitor.visit_address(depth, *xs.at(ndx)?),
        }
    }
}
        ...
        impl ValueView for ElemView<'_> {
            fn visit(&self, visitor: &mut impl ValueVisitor) -> PartialVMResult<()> {
                match &self.container {
                    Value::Vec(v) => v.at(self.ndx)?.borrow().visit(visitor),
    ...
    pub fn as_move_value(&self, layout: &MoveTypeLayout) -> PartialVMResult<RuntimeValue> {
        ...
        Ok(match (layout, self) {
            ...
            (L::Enum(enum_layout), Value::Variant(entry)) => {
                ...
                let field_layouts = variants.at(tag as usize)?;
                let mut fields = vec![];
