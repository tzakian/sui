...
fn signer_equivalence() -> PartialVMResult<()> {
    ...
    assert_eq!(
        signer.serialize(),
        signer.typed_serialize(&runtime_value::compressed_layouts::MoveTypeLayout::from(
    ...
    assert_eq!(
        signer.serialize(),
        signer.typed_serialize(&runtime_value::compressed_layouts::MoveTypeLayout::from(
            &runtime_value::MoveTypeLayout::Struct(Box::new(runtime_value::MoveStructLayout(
                Box::new(vec![runtime_value::MoveTypeLayout::Address]),
            ))),
        ))
    );
