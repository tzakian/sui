...
use crate::data_store::{PackageStore, ResolvablePackageStore};
...
impl<'state> CachedPackageStore<'state> {
    ...
    pub fn push_package(&self, id: ObjectID, package: Rc<MovePackage>) -> SuiResult<()> {
        ...
        if let Some(_) = self.new_packages.borrow_mut().insert(id, package) {
...
impl ResolvablePackageStore for CachedPackageStore<'_> {
    // TODO: remove when we upgrade to Rust 1.86
