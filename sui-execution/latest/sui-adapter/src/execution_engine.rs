...
mod checked {
    ...
    use sui_types::execution_status::{CongestedObjects, ExecutionFailureStatus, ExecutionStatus};
    ...
    fn compare_effects<Mode: ExecutionMode>(
        ...
        let ok = match (normal_effects.2.status(), new_effects.2.status()) {
            ...
            (
                ...
                failures_differ = error == other_error;
                true
            }
        ...
        if !ok {
            ...
            std::fs::write(
                &format!("outputs/{}", normal_effects.2.transaction_digest()),
                &data,
            )
