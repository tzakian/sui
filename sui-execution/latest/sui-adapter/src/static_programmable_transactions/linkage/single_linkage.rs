...
use crate::{
    ...
    static_programmable_transactions::{
        linkage::{
            ...
            resolution::{ResolutionTable, VersionConstraint, add_and_unify, add_package},
            resolved_linkage::{ExecutableLinkage, ResolvedLinkage},
        },
        loading::ast::{Command, LoadedFunction, PackagePayload, Transaction},
    },
};
...
pub fn refine_to_single_linkage<E: ExecutionErrorTrait>(
    ...
    for command in &mut txn.commands {
        write_back_linkage::<E>(command, &resolved_linkage)?;
    }
...
fn analyze_command<E: ExecutionErrorTrait>(
    ...
    match command {
        ...
        // NB: we don't count the type in the MakeMoveVec as a dependency, so we don't need to
        // resolve it here. But this is debatable.
