...
pub fn trace_split_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        update_coin_balance(&mut input, |current_balance| {
            current_balance - total_split_value
        })?;
...
pub fn trace_merge_coins(
    ...
    if let Some(trace_builder) = trace_builder_opt {
        ...
        update_coin_balance(&mut target_coin_resulting_state, |current_balance| {
            current_balance + total_merged_value
        })?;
...
fn serializable_move_value_from_ctx_value(
    ...
    VMValue::as_annotated_move_value_for_tracing_only(&value.0.0, annotated_layout)
        .ok_or_else(|| ExecutionError::from_kind(ExecutionErrorKind::InvariantViolation))
        .map(SerializableMoveValue::from)
}

// Ugly ugly... but we need to alter the balance in the input coin to reflect
// the split/merge operations.
