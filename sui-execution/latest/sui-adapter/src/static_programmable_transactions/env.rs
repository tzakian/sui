...
use move_vm_types::loaded_data::runtime_types as vm_runtime_type;
...
pub struct Env<'pc, 'vm, 'state, 'linkage> {
    ...
    pub linkable_store: &'linkage dyn ResolvablePackageStore,
...
impl<'pc, 'vm, 'state, 'linkage> Env<'pc, 'vm, 'state, 'linkage> {
    pub fn new(
        ...
        linkable_store: &'linkage dyn ResolvablePackageStore,
    ...
    pub fn convert_vm_error(&self, e: VMError) -> ExecutionError {
        convert_vm_error(
            e,
            self.linkable_store,
            &self.linkage_analysis.config().binary_config,
        )
    }
    ...
    pub fn fully_annotated_layout(
        ...
        let (ty, _) = self.vm_type_from_adapter_type(ty)?;
        self.vm
            .get_runtime()
    ...
    pub fn load_function(
        ...
        let loaded_type_arguments = type_arguments
            .iter()
            .map(|ty| self.vm_type_from_adapter_type(ty).map(|(ty, _)| ty))
    ...
    pub fn load_type_input(&self, _idx: usize, ty: TypeInput) -> Result<Type, ExecutionError> {
        let (runtime_type, linkage) = self.vm_type_from_type_input(ty)?;
    ...
    pub fn load_type_from_struct(&self, tag: &StructTag) -> Result<Type, ExecutionError> {
        let (vm_type, linkage) =
            self.vm_type_from_type_tag(&TypeTag::Struct(Box::new(tag.clone())))?;
...
// TODO/XXX: We need to update the VM so that it returns the relocated vm error (i.e., resolves
// abort codes/locations to their relocated versions and not using the runtime id).
fn convert_vm_error(
    error: VMError,
    ...
    let kind = match (error.major_status(), error.sub_status(), error.location()) {
        ...
        (StatusCode::ABORTED, Some(code), Location::Module(id)) => {
            ...
            let function_name = store
                .get_package(&ObjectID::from(*abort_location_id.address()))
                .ok()
                .flatten()
                .and_then(|p| {
                    p.deserialize_module_by_str(abort_location_id.name().as_str(), binary_config)
                        .ok()
                })
                .map(|module| {
            ...
            ExecutionFailureStatus::MoveAbort(
                MoveLocation {
                    module: abort_location_id.clone(),
                    function,
                    instruction,
                    function_name,
                },
                ...
                let location = match location {
                    Location::Module(id) => {
                        ...
                        let function_name = store
                            ...
                            .flatten()
                            .and_then(|p| {
                                p.deserialize_module_by_str(id.name().as_str(), binary_config)
                                    .ok()
                            })
                            .map(|module| {
                                let fdef =
