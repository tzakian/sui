...
macro_rules! gated {
    ($self:ident) => {
        if !$self.protocol_config.enable_ptb_compilation_metering() {
            return Ok(());
        }
    };
}
...
impl<'gas, 'pc> TransactionMeter<'gas, 'pc> {
    ...
    pub fn charge_command_base(&mut self) -> Result<(), ExecutionError> {
        gated!(self);
        self.charger
            .move_gas_status_mut()
    ...
    pub fn charge_package_load(&mut self, package: &MovePackage) -> Result<(), ExecutionError> {
        gated!(self);
