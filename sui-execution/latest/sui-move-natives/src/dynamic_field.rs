...
pub fn borrow_child_object(
    ...
    if let Some(charge) = charge_cache_or_load_gas(
        get_extension!(context, ObjectRuntime)?.protocol_config,
        &cache_info,
    ) {
        native_charge_gas_early_exit!(context, charge);
    }
...
pub fn remove_child_object(
    ...
    if let Some(charge) = charge_cache_or_load_gas(protocol_config, &cache_info) {
        native_charge_gas_early_exit!(
