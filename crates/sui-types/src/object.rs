...
use crate::error::{
    ExecutionError, SuiError, SuiErrorKind, SuiResult, UserInputError, UserInputResult,
};
...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<CA::MoveStructLayout, SuiError> {
    ...
    pub fn get_struct_layout_from_struct_tag(
        ...
        match layout.as_view() {
            CA::MoveLayoutView::Struct(l) => Ok(*l),
            _ => unreachable!(
    ...
    pub fn to_move_struct(&self, layout: CA::MoveStructLayout) -> Result<MoveStruct, SuiError> {
...
impl ObjectInner {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<CA::MoveStructLayout>, SuiError> {
        match &self.data {
