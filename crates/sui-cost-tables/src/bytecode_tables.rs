...
impl<'a> GasStatus<'a> {
    ...
    pub fn push_stack(&mut self, pushes: u64) -> PartialVMResult<()> {
        match self.stack_height_current.checked_add(pushes) {
            ...
            None => return Err(PartialVMError::new(StatusCode::VALUE_STACK_OVERFLOW)),
            Some(new_height) => {
    ...
    pub fn pop_stack(&mut self, pushes: u64) -> PartialVMResult<()> {
        match self.stack_height_current.checked_sub(pushes) {
            None => {
                self.stack_height_current = 0;
            }
            Some(new_height) => {
                self.stack_height_current = new_height;
            }
    ...
    pub fn decrease_stack_size(&mut self, size_amount: u64) -> PartialVMResult<()> {
        match self.stack_size_current.checked_sub(size_amount) {
            None => {
                self.stack_size_current = 0;
            }
            Some(new_size) => {
                if new_size > self.stack_size_high_water_mark {
                    self.stack_size_high_water_mark = new_size;
    ...
    pub fn charge(
        ...
        self.deduct_gas(
            GasCost::new(
                self.instructions_current_tier_mult * num_instructions,
                self.stack_size_current_tier_mult * Into::<u64>::into(incr_size),
                self.stack_height_current_tier_mult * pushes,
            )
            .total_internal(),
        )?;
...
impl<'b> GasMeter for GasStatus<'b> {
    ...
    fn charge_ld_const(&mut self, size: NumBytes) -> PartialVMResult<()> {
        ...
        self.charge(1, 1, 0, u64::from(size).into(), 0)
    }
    ...
    fn charge_borrow_global(
        ...
        // We don't have `borrow_global` so don't need to account for it.
        Ok(())
    }
    ...
    fn charge_exists(
        ...
        // We don't have `exists` so don't need to account for it.
        Ok(())
    }
    ...
    fn charge_move_from(
        ...
        // We don't have `move_from` so don't need to account for it.
        Ok(())
    }
    ...
    fn charge_move_to(
        ...
        // We don't have `move_to` so don't need to account for it.
        Ok(())
    }
    ...
    fn charge_vec_unpack(
        &mut self,
        _ty: impl TypeView,
        // TODO(tzakian): WHY DO WE NEED THIS??? Shouldn't this be the same size as `elems`?
        expect_num_elements: NumArgs,
        _elems: impl ExactSizeIterator<Item = impl ValueView>,
    ) -> PartialVMResult<()> {
        // Charge for the pushes
        let pushes: u64 = u64::from(expect_num_elements).into();
...
fn get_simple_instruction_stack_change(
    ...
    match instr {
        ...
        CastU16 => (1, 1, Type::U16.size(), Type::U16.size()),
        CastU32 => (1, 1, Type::U32.size(), Type::U32.size()),
        CastU64 => (1, 1, Type::U64.size(), Type::U64.size()),
        CastU128 => (1, 1, Type::U128.size(), Type::U128.size()),
        CastU256 => (1, 1, Type::U256.size(), Type::U256.size()),
        ...
        Or | And => (2, 1, Type::Bool.size() + Type::U8.size(), Type::Bool.size()),
