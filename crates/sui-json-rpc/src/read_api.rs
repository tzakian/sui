                ...
                ObjectRead::Exists(object_ref, o, layout) => {
                    ...
                    if options.show_display {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                let layout = layout.map(compress_struct_layout).transpose()?;
...
/// Compress a tree-form `MoveStructLayout` into its compressed equivalent.
fn compress_struct_layout(layout: MoveStructLayout) -> anyhow::Result<CA::MoveStructLayout> {
    let built = CA::MoveTypeLayoutBuilder::with_builder(|b| b.from_tree_struct_layout(&layout))?;
    let CA::MoveLayoutView::Struct(s) = built.as_view() else {
        anyhow::bail!("Expected struct layout");
    };
    Ok(*s)
}
