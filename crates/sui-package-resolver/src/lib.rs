...
impl<'l> ResolutionContext<'l> {
    ...
    fn resolve_type_layout(
        ...
        Ok(match tag {
            ...
            T::Address => (builder.address(), 1),
            T::Bool => (builder.bool(), 1),
            T::U8 => (builder.bool(), 1),
            T::U16 => (builder.u16(), 1),
            T::U32 => (builder.u32(), 1),
            T::U64 => (builder.u64(), 1),
            T::U128 => (builder.u128(), 1),
            T::U256 => (builder.u256(), 1),

            T::Vector(tag) => {
                let (layout, depth) = self.resolve_type_layout(builder, tag, max_depth - 1)?;
                (
                    unable_to_construct_if_err(builder.vector(layout))?,
                    depth + 1,
                )
            }
            ...
            T::Struct(s) => {
                ...
                let param_layouts = s
                    ...
                    // within this struct.
                    .map(|tag| self.resolve_type_layout(builder, tag, max_depth - 1))
                ...
                let type_params = param_layouts
                    .iter()
                    .map(|l| {
                        unable_to_construct_if_none(
                            builder.type_tag(l.0),
                            "Unable to derive type tag from type layout",
                        )
                    })
                    .collect::<Result<_>>()?;
                ...
                self.resolve_datatype_signature(builder, def, type_, param_layouts, max_depth)?
            }
        })
    }
    ...
    fn resolve_datatype_signature(
        ...
        data_def: &DataDef,
        type_: StructTag,
        param_layouts: Vec<(LayoutHandle, usize)>,
        max_depth: usize,
    ) -> Result<(LayoutHandle, usize)> {
        Ok(match &data_def.data {
            MoveData::Struct(fields) => {
                ...
                for (name, sig) in fields {
                    let (layout, depth) =
                        self.resolve_signature_layout(builder, sig, &param_layouts, max_depth - 1)?;

                    field_depth = field_depth.max(depth);
                    resolved_fields.push((ident(name.as_str())?, layout))
                }

                (
                    unable_to_construct_if_err(builder.struct_layout(type_, resolved_fields))?,
                    field_depth + 1,
                )
            }
            MoveData::Enum(variants) => {
                ...
                for (tag, variant) in variants.iter().enumerate() {
                    ...
                    for (name, sig) in &variant.signatures {
                        ...
                        let (layout, depth) = self.resolve_signature_layout(
                            builder,
                            sig,
                            &param_layouts,
                            max_depth - 1,
                        ...
                        field_depth = field_depth.max(depth);
                        fields.push((ident(name.as_str())?, layout))
                    }
                    resolved_variants.push((
                        ident(variant.name.as_str())?,
                        tag as u16,
                        Some(fields),
                    ));
                }
                ...
                (
                    unable_to_construct_if_err(builder.enum_layout(type_, resolved_variants))?,
                    field_depth + 1,
                )
            }
        })
    ...
    fn resolve_signature_layout(
        ...
        builder: &mut CA::MoveTypeLayoutBuilder,
        sig: &OpenSignatureBody,
        param_layouts: &[(LayoutHandle, usize)],
        max_depth: usize,
    ) -> Result<(LayoutHandle, usize)> {
        use OpenSignatureBody as O;
        ...
        Ok(match sig {
            O::Address => (builder.address(), 1),
            O::Bool => (builder.bool(), 1),
            O::U8 => (builder.u8(), 1),
            O::U16 => (builder.u16(), 1),
            O::U32 => (builder.u32(), 1),
            O::U64 => (builder.u64(), 1),
            O::U128 => (builder.u128(), 1),
            O::U256 => (builder.u256(), 1),
            ...
            O::Vector(sig) => {
                ...
                (
                    builder
                        .vector(layout)
                        .map_err(|e| Error::UnableToConstructLayout(Arc::new(e.to_string())))?,
                    depth + 1,
                )
            }
            ...
            O::Datatype(key, params) => {
                ...
                let type_params: Vec<TypeTag> = param_layouts
                    .iter()
                    .map(|l| {
                        builder.type_tag(l.0).ok_or_else(|| {
                            Error::UnableToConstructLayout(Arc::new(
                                "Unable to derive type tag from type layout".to_string(),
                            ))
                        })
                    })
                    .collect::<Result<_>>()?;
...
fn unable_to_construct_if_err<T, E: ToString>(res: std::result::Result<T, E>) -> Result<T> {
    res.map_err(|e| Error::UnableToConstructLayout(Arc::new(e.to_string())))
}

fn unable_to_construct_if_none<T>(res: Option<T>, msg: &str) -> Result<T> {
    res.ok_or_else(|| Error::UnableToConstructLayout(Arc::new(msg.to_owned())))
}
