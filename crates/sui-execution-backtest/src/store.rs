...
pub struct PackageCache {
    ...
    handle: Handle,
    disk: Option<PathBuf>,
    mem: Mutex<HashMap<ObjectID, Option<Object>>>,
}
...
impl PackageCache {
    ...
    fn fetch(&self, id: ObjectID) -> Option<Object> {
        if let Some(hit) = self.mem.lock().unwrap().get(&id) {
            return hit.clone();
        }
        ...
        if let Some(path) = self.disk_path(id) {
            if let Ok(bytes) = std::fs::read(&path) {
                if let Ok(object) = bcs::from_bytes::<Object>(&bytes) {
                    self.mem.lock().unwrap().insert(id, Some(object.clone()));
                    return Some(object);
                }
            }
        }
        ...
        self.mem.lock().unwrap().insert(id, fetched.clone());
        fetched
    }
}
...
pub fn resolve_input_objects(
    ...
    let shared_versions: BTreeMap<ObjectID, SequenceNumber> = effects
            ...
            InputObjectKind::SharedMoveObject { id, .. } => {
                ...
                let object = match shared_versions.get(&id) {
                    Some(&version) => store.get_object_by_key(&id, version).with_context(|| {
                        format!("shared input object {id} v{version} not in object set")
                    })?,
                    None => store
                        .get_object(&id)
                        .with_context(|| format!("shared input object {id} not in object set"))?,
                };
                resolved.push(ObjectReadResult {
                    input_object_kind: kind,
                    object: ObjectReadResultKind::Object(object),
