...
>(
    ...
    let (gas_ref, result) = if !gas_status.is_unmetered() && gas_object_ref.len() > 1 {
        match smash_gas_coins(&tx_ctx, gas_object_ref, &mut temporary_store) {
            ...
            // can be used for gas
            Err(err) => (gas_object_ref[0], Err(err)),
        }
    } else {
        (gas_object_ref[0], Ok(()))
    };
