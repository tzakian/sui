...
fn test_convert_address_from_bcs() {
    ...
    let value = SuiJsonValue::from_bcs_bytes(Some(&compress(&MoveTypeLayout::Signer)), &bcs_bytes)
        .unwrap();
...
fn test_sui_call_arg_option_type() {
    ...
    let v =
        SuiJsonValue::from_bcs_bytes(Some(&compressed_option_layout), &arg1).unwrap();
